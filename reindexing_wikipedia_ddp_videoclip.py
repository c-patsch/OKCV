...
def reindex(rank, machine_rank, world_size, master_addr, master_port, model_setup, tokenizer_class, tokenizer_parameters, model_path=None, overall_rank=None, filename='data/wikipedia_split/psgs_w100.tsv', file_size=21015324):
    if world_size > 0:
    ...
    with torch.no_grad():
        with tqdm(total=file_size//world_size) as pbar:
            for i, batch in enumerate(read_tsv_lazy(filename, tokenizer, max_tokens, overall_rank if overall_rank else machine_rank+rank, world_size)):
                inputs = tokenizer(["title: " + title + " passage: " + text[1:] for title, text, _ in batch], return_tensors="pt", truncation=True, padding=True, max_length=77)
    ...
    pkl.dump(data, open(f"wikipedia_embeddings/wikipedia_embeddings_dpr_{overall_rank if overall_rank else machine_rank+rank}.pkl", "wb"))



def run_index(world_size, master_addr, master_port, machine_index, model_setup, model_path=None, embeddings_name=None, filename='data/wikipedia_split/psgs_w100.tsv', file_size=21015324):
